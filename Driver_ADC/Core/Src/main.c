...
#define MOISTURE_THRESHOLD_OPTIMAL  70
...
int main(void) {
    ...
    I2C_Init();
    DS3231_setTime(15, 5, 30);
    DS3231_setDate(2, 2, 6, 25);
...
void GPIO_pinsConfig(void) {
    ...
    gpio_config_t lcd_pins[] = { { GPIOA, 5, GPIO_DRIVER_MODE_OUTPUT, GPIO_DRIVER_OUTPUT_PUSH_PULL,
            ...
            { GPIOA, 7, GPIO_DRIVER_MODE_OUTPUT, GPIO_DRIVER_OUTPUT_PUSH_PULL, GPIO_DRIVER_SPEED_MEDIUM,
                    GPIO_DRIVER_NO_PULL },
            { GPIOA, 8, GPIO_DRIVER_MODE_OUTPUT, GPIO_DRIVER_OUTPUT_PUSH_PULL, GPIO_DRIVER_SPEED_MEDIUM,
                    GPIO_DRIVER_NO_PULL },
            { GPIOA, 9, GPIO_DRIVER_MODE_OUTPUT, GPIO_DRIVER_OUTPUT_PUSH_PULL, GPIO_DRIVER_SPEED_MEDIUM,
                    GPIO_DRIVER_NO_PULL },
            { GPIOA, 10, GPIO_DRIVER_MODE_OUTPUT, GPIO_DRIVER_OUTPUT_PUSH_PULL, GPIO_DRIVER_SPEED_MEDIUM,
                    GPIO_DRIVER_NO_PULL }
    };
...
void handleButtonInputs(void) {
    ...
    if (current_mode == MANUAL_MODE) {
        ...
        case SETTING_STOP_HOUR:
            ...
            if (readButtonDebounced(DOWN_BUTTON_PORT, DOWN_BUTTON_PIN)) {
                manual_stop_time.hours = (manual_stop_time.hours + 23) % 24;
            }
            if (readButtonDebounced(RIGHT_BUTTON_PORT, RIGHT_BUTTON_PIN)) {
        ...
        case SETTING_STOP_MINUTE:
            ...
            if (readButtonDebounced(DOWN_BUTTON_PORT, DOWN_BUTTON_PIN)) {
                manual_stop_time.minutes = (manual_stop_time.minutes + 59) % 60;
            }
            if (readButtonDebounced(RIGHT_BUTTON_PORT, RIGHT_BUTTON_PIN)) {
                manual_ui_state = DISPLAY_MANUAL_NORMAL;
            }
...
void processAutoMode(void) {
    ...
    for (int i = 0; i < sizeof(adc_buffer) / sizeof(uint16_t); i++) {
        soil_moisture_raw += adc_buffer[i];
    }
    soil_moisture_raw /= (sizeof(adc_buffer) / sizeof(uint16_t));
    soil_moisture_percent = ADC_convertToMoisturePercentage(soil_moisture_raw);
